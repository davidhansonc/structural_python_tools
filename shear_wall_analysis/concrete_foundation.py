...
class shear_wall_footing():
    ...
    def __init__(self, h=10.0, L=12.0, L_w=10.0, D=18.0, t=12.0):
        self.L = L
        self.L_1 = (L - L_w) / 2
        self.L_w = L_w
        self.D = D
        self.beam = Beam()
        self.update_beam_units()

        
    
    def update_beam_units(self, beam):
        self.beam.update_units(key='length', unit='ft')
        self.beam.update_units('force', 'lbf')
        self.beam.update_units('distributed', 'lbf/ft')
        self.beam.update_units('moment', 'lbf.ft')
